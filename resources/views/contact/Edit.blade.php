...
<body class="bg-slate-50 min-h-screen p-8 font-sans">
    <div class="max-w-lg mx-auto bg-white rounded-2xl shadow-md border border-slate-200 p-6">
        <h1 class="text-2xl font-bold mb-4 text-slate-800">Edit Contact</h1>
        ...
        <form action="{{ route('contacts.update', $contact->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="phone" value="{{ $contact->phone }}" class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-200">
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Editor ID</label>
                <input type="text" name="editor_id" value="{{ $contact->editor_id }}" class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-200">
            </div>
